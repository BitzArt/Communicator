    ...
    where TModel : class
{
    ...
    public override async Task<TResponse> UpdateAsync<TResponse>(TKey? id, TModel model, IFluxRequestParameters parameters, bool partial = false, CancellationToken cancellationToken = default)
    ...
    public override Task<TResponse> UpdateAsync<TResponse>(TModel model, IFluxRequestParameters parameters, bool partial = false, CancellationToken cancellationToken = default)
        => UpdateAsync<TResponse>(id: default, model, parameters, partial, cancellationToken);

    private async Task<TResponse> UpdateAsyncInternal<TResponse>(TKey? id, TModel model, IFluxRequestParameters? parameters, bool partial, CancellationToken cancellationToken)
    {
